...
export function AccountMenu({
  ...
  useEffect(() => {
    fetch('/api/social/summary')
      .then((r) => (r.ok ? (r.json() as Promise<{ pendingFriends: number }>) : null))
      .then((d) => {
        if (d) setSocial(d);
      })
      .catch(() => {
    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 flex max-h-[min(80vh,32rem)] w-[19.5rem] flex-col overflow-hidden rounded-2xl border border-stone-200/80 bg-paper/95 shadow-xl backdrop-blur-md dark:border-stone-700/80 dark:bg-stone-900/95">
          ...
          {/* Быстрый раздел: друзья */}
          <div className="grid shrink-0 grid-cols-1 gap-1.5 border-b border-stone-200/70 px-3 py-2 dark:border-stone-800/70">
            <Link
              href="/friends"
