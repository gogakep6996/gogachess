...
export function ClassPublicClient({ meId, cls, initialTasks, isOwner }: Props) {
  ...
  const activeBoard = useMemo(() => {
    ...
    if (state.lessonActive && state.demoRoomCode) {
    ...
    <>
      <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">Учитель: {cls.ownerName}</p>
        </div>
        {isOwner && (
      ...
      {/* Баннер активного урока */}
      {state?.lessonActive && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm dark:border-emerald-700 dark:bg-emerald-900/30">
          <div className="flex items-center gap-2 font-semibold text-emerald-700 dark:text-emerald-200">
            <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-emerald-500" />
            {activeBoard?.kind === 'demo'
              ? 'Учитель ведёт показ всем — следите за доской ниже'
              : activeBoard?.kind === 'task'
                ? `Ваша задача: ${activeBoard.taskTitle}`
                : 'Идёт урок. Учитель раздаст задачу — она появится здесь.'}
          </div>
          {!meId && (
      ...
      {/* Встроенная доска: личная задача ученика ИЛИ демо учителя */}
      {activeBoard && meId && (
        <div className="mb-4">
          <LessonBoard
            roomCode={activeBoard.roomCode}
            meId={meId}
            variant={activeBoard.kind}
            humanColor={
              activeBoard.kind === 'task'
                ? ((currentTaskForSide?.sideToPlay as 'w' | 'b') ?? 'w')
                : undefined
            }
            engineSkill={currentTaskForSide?.engineLevel ?? 10}
            caption={
              activeBoard.kind === 'demo'
                ? 'Показ учителя'
                : currentTaskForSide?.title
            }
          />
        </div>
      )}

      {/* Общий аудио + чат класса (когда урок активен) */}
      ...
      {codeNeeded ? (
        ...
        <>
          <h2 className="mb-3 text-base font-semibold">Задачи учителя</h2>
          {tasks.length === 0 ? (
          ...
          <p className="mt-4 text-center text-xs text-stone-500">
            Дождитесь начала урока — учитель раздаст задачу, и она откроется на вашей личной
            доске.
          </p>
        </>
      )}
