...
/** Проверяет, что библиотечная папка принадлежит классу текущего учителя. */
...
// PATCH /api/class/me/library-folders/[id] — переименовать { name } / порядок { position }.
export async function PATCH(
  ...
  const folder = await prisma.libraryFolder.update({ where: { id }, data });
  return NextResponse.json({ folder });
}

// DELETE /api/class/me/library-folders/[id] — удалить папку.
// Позиции не удаляются: связи задача↔папка (LibraryFolderTask) убираются
// каскадом (onDelete: Cascade), и такие позиции становятся «Без папки».
