...
### Капча (обязательно на публичном проде)

Без капчи регистрация открыта для ботов, а любой зарегистрировавшийся может
создать класс и получить доступ к ученикам. Используется Yandex SmartCaptcha:
российский сервис, данные посетителей не уходят за границу.

1. Зайдите в [console.yandex.cloud](https://console.yandex.cloud) → сервис
   **SmartCaptcha** → **Создать капчу**.
2. Укажите домен `gogachess.ru`, тип — обычная капча (с кнопкой «Я не робот»).
3. Скопируйте два ключа: **ключ клиентской части** (публичный) и
   **ключ сервера** (секретный).
4. Добавьте в `.env` на сервере:
   ```env
   SMARTCAPTCHA_SERVER_KEY=ysc2_...
   NEXT_PUBLIC_SMARTCAPTCHA_CLIENT_KEY=ysc1_...
   ```
5. Пересоберите контейнер (`docker compose up -d --build`) — виджет
   автоматически появится на `/register`.

Добавлять оба ключа нужно одной правкой. `NEXT_PUBLIC_SMARTCAPTCHA_CLIENT_KEY`
попадает в клиентский код на этапе сборки и передаётся через `build.args`
в `docker-compose.yml`, а `SMARTCAPTCHA_SERVER_KEY` читается при запуске. Если
задать только серверный, бэкенд начнёт требовать токен, которого виджет без
клиентского ключа не выдаст, и вход с регистрацией перестанут работать у всех.
По той же причине смена клиентского ключа требует именно пересборки: рестарт
контейнера её не подхватит.

Проверка: откройте `/register` в режиме инкогнито, под формой должна быть
кнопка «Я не робот». Попытка отправить форму без неё вернёт ошибку
«Не удалось пройти проверку „я не бот“».

Ключ клиентской части попадает в сборку страницы, поэтому он публичный по
природе — это нормально. Ключ сервера в репозиторий и в чаты не передавать.
